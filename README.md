...
Components v2(`LayoutView`)의 **구분선(Separator)** 과 텍스트로 **3구역**이 시각적으로 나뉩니다.

| 구역 | UI |
|------|-----|
| 1️⃣ 팀 구성 | `2:2`, `3:3`, `4:4`, `5:5`, `2:2:2:2` 버튼 + **직접입력** Modal |
| 2️⃣ 멤버 선택 | **멤버 선택** 버튼 → 비공개 버튼 목록(12명/페이지, 3×4) |
| 3️⃣ 실행 | 🎲 팀 나누기 · 선택 해제 · 🔄 초기화 |

| 기능 | 설명 |
|------|------|
| 직접입력 | Modal에서 `3:3:2` 등 자유 형식 입력 |
| 멤버 선택 | 봇을 제외한 전체 멤버를 이전/다음으로 조회, 선택 멤버는 텍스트 영역에 표시 |
| 멤버 검색 | **검색** 버튼으로 Modal을 열어 닉네임·사용자명 검색 |
| 이름 표시 | 최대 7자·`…` 처리 후 양쪽 공백으로 폭을 맞추며, 선택은 초록색으로 표시 |
| 팀 나누기 | 선택 인원 = 구성 합계일 때 랜덤 배정 |
| 결과 | A/B/…팀 LayoutView, **다시 섞기**·**멤버 수정** |
...
알림 채널을 처음 설정하면 현재 최신 패치 1건을 올리고 본문 스냅샷을 저장합니다. 이후 자동 폴링은 **새 패치**와 기존 패치 글의 **추가 패치(핫픽스)** 만 알립니다. 확인 주기는 기본 30분(`TFT_POLL_MINUTES`)입니다.
